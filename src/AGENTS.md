# src/AGENTS.md · 生成引擎的模块规矩（开发 Agent 的唯一入口）

## 0. 你在改什么？（每次动手前必答）

| 你要改的路径 | 层 | 规矩 |
|---|---|---|
| `spec/**` `profiles/**` `brands/**` `cases/export/**` | **资产 A**（永不丢弃） | 必须开 ADR；必须有 evidence；必须过 `make arch` |
| `src/**` `tests/**` | **代码 B2**（可丢弃） | 自由重写，但必须让 `tests/` 全绿；受行数预算约束 |
| `out/**` `docs/metrics/**` | **生成物 B3** | gitignored，运行时产出 |

## 1. 工作循环（TDD，强制）
1. 先写/取测试：`tests/` 下对应文件。**测试必须先红。**
2. 实现最小代码让测试绿。
3. `make check` 全绿。
4. 提 PR，填 `.github/pull_request_template.md`。若改了资产层，附 ADR 链接。
...
- 禁止在 prompt/代码里硬编码自然语言知识。这类知识去 `spec/rubrics/` 或 `spec/rules/L3_canonical/`。
