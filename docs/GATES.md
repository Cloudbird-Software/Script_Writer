...
|---|---|---|---|---|---|
| **L0-a 结构** | Pydantic + 16 条不变量 | `check`（gate 聚合） | ✅ | 20s | $0 |
| **L0-b 约束** | 34+ 条声明式规则 | `check`（gate 聚合） | ✅ | 30s | $0 |
| **L0-c 资产纪律** | D2 归约 / prompts 未手改 / 行数预算 / 规则冲突 | `check`（make arch） | ✅ | 15s | $0 |
| **L0-d 黄金回归** | 渲染快照 + 锚点往返 | `check`（make test 含 golden） | ✅ | 40s | $0 |
...
## 本地必须跑通才提 PR
```bash
make check        # = lint + typecheck + arch(spec-guard) + test
```
