...
1. 认证：一切 push/PR 用 cloudbrid-agent App 令牌，禁个人 PAT。获取：
   `GH_TOKEN=$(REPO=Script_Writer bash <(curl -sS https://raw.githubusercontent.com/Cloudbird-Software/.github/f72d9520706c8fca974d92456f65cae5c1412bb7/scripts/gh-app-token.sh))`
...
| 加/改门禁（五道硬门） | `internal/rules/gates/`（各域子包 `internal/rules/<域>/`）包注释 + [issue #1](https://github.com/Cloudbird-Software/Script_Writer/issues/1) §B-2 |
| 改全局 pass / 报表 / CLI | `internal/engine/`（全局 pass/报表）+ `cmd/songguard/`（CLI，经 `internal/songguard/` 门面） |
